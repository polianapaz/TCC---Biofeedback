...
porta='COM'+str(port)
print(porta)
ser=serial.Serial(porta, baudrate = 500000, timeout=0.01)
...
class PacienteDados:
    ...
    def data_entry(self, nome, idade, tipo_trat, data, posimax, posimin, pontos, tempo, grafico, modo = 'Exercício'):
        db_obj_cursor.execute('CREATE TABLE IF NOT EXISTS dados (paciente text, idade integer, \
                                ...
                                pontos text, tempo text, nome_grafico text, modo text)')
        hora = ('{}-{}-{} {}:{}:{}'.format(data.day, data.month, data.year, data.hour, data.minute, data.second))
        
        db_obj_cursor.execute("INSERT INTO dados VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [nome, idade, \
                                tipo_trat, hora, posimax, posimin, pontos, tempo,  grafico, modo])
        db_obj.commit()
    ...
    def grafico_select(self, nome, data, tipo_trat, modo = "Exercício"):
        tipo_tra = []
        sql = 'SELECT nome_grafico FROM dados WHERE paciente = ? and data = ? and tratamento = ? and modo = ?'
        for row in db_obj_cursor.execute(sql, [nome, data, tipo_trat, modo]):
            tipo_tra.append(row[0])
        return tipo_tra
    
    def dados_select(self, nome, data, tipo_trat, modo = 'Exercício'):
        x, y = [], []
        sql = 'SELECT pontos, tempo FROM dados WHERE paciente = ? and data = ? and tratamento = ? and modo = ?'
        for row in db_obj_cursor.execute(sql, [nome, data, tipo_trat, modo]):
            x.append(row[0])
            y.append(row[1])
        return x,y
    ...
    def pressao_maxmin_select(self, nome, modo = 'Taragem'):
        for minimo in db_obj_cursor.execute("SELECT pressao_max FROM dados WHERE paciente = ? and modo = ?" , [nome, modo]):
            mini = minimo[0]
        for maximo in db_obj_cursor.execute("SELECT pressao_min FROM dados WHERE paciente = ? and modo =? ", [nome, modo]):
            maxi = maximo[0]
        return mini,maxi
...
def gameover(pont, temp, paciente, data, tratamento):
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    ...
                    if posibot == 11:
                        ...
                        resultados(pont, temp, paciente, data, tratamento)
                    elif posibot == 26:
                        historico()
                    else:
                        final = False
    ...
    while janelamax:
        ...
        if event in (None, 'Voltar'):
            ...
            if tara == True:
                ...
                bd_geral.data_entry(n_paciente[0], idade_paciente[0], trata_paciente[0], dt, 0, 0, "DEFAULT", "DEFAULT", "DEFAULT", "Taragem")
...
#####################
    ...
    while tara:
        ...
        while (contagem2 <= 10):
            ...
            if (ser.inWaiting() > 0):
                teste = int(ser.read(ser.inWaiting()))
                ##print("Teste = ",teste)
            ...
            with open('Medidas_comCapacitor_semMedia_livre.txt', 'a', errors = 'ignore') as arquivo:
                arquivo.write("%s\n" %posi)
            arquivo.close()
...
def resultados(pontos, tempo, paciente, data, trat): #DETALHES
                        ...
                        if data[0] != "":
                            ...
                            if event in (None, 'Cancelar'):
                                ...
                                if trat[0] != "":
                                    ...
                                    n_graf = bd_geral.grafico_select(paciente[0],datasel[0],graf_selec[0])
                                    janela = False
                                    hist = False
                                    resultados(dados[0][0], dados[1][0], paciente, datasel, n_graf[0])
                                else:
                                    pass
                        else:
                            pass
...
def progresso():
    ...
    pos_buble_y, col, cor, posibot, grafico = 700, coluna(), coral(), 11, grafmmae()
...
def grafico(graf, tempototal):
    ...
    pontos_result, pos_col_y_buffer, pos_col_y, cont_aux = [], [], [], []
    sair, pontos, pausa, musica, colidiu, cont, escolha, flag, tempo_desloc0, tempo_desloc1 = \
    True, 0, 1, True, 0, 0, 0, 0, 0, 0
...
####### CONTROLE DE MOVIMENTOS DO OBSTÁCULO #################
        ...
        if contagem < 0.3:
            nova_posi = posmax[0] - (int(altura - posmin[0])*(posmax[0]-posmin[0])/altura) 
        else:  
            nova_posi = posmax[0] - (int(altura - pos_y)*(posmax[0]-posmin[0])/altura)
        
        pontos_result.append(round(nova_posi)) #Lista com pontos por onde a sereia passou.
...
##                escolha = 1
        ...
        if altura_col(altcol[graf-1]) != "Trabalho Livre.txt":
            for i in range (5):
                if (pos_x >= pos_obs_x[i]-50 and pos_x <= pos_obs_x[i]+50) and (pos_y>=(pos_obs_y[i]+400) or pos_y<=(pos_obs_y[i]+250)):
        ...
        if sair == True:
            if contagem < 0.3:
                ...
                ariel.show(tela, pos_x, altura - pos_y, escolha)
