...
class fundo(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.imagemfundo = pygame.transform.scale(pygame.image.load('imagens/fund1.gif'), (int(largura+10), int(altura+10)))
    ...
    def show(self, tela, pos_x, pos_y, imagem):
        if imagem==1:
            tela.blit(self.imagemfundo, self.rect) 
            self.rect.centerx, self.rect.centery = pos_x, pos_y
        if imagem==2:   
...
class fish (pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.imagempeixe3 = pygame.transform.scale(pygame.image.load('imagens/peixe1.png'), (int(largura/50), int(altura/30)))
...
def grafico(graf):
    ...
    pos_obs_x, pos_obs_x2, pos_obs_x3, velocidade_obs = larg, larg, larg, 10
    ...
    sair, pontos, pausa, musica, colidiu, cont = True, 0, 1, True, 0, 0
    pos_col_y=[]
    pos_col_y = altura_col("Contração Tônica.txt")
    #player funciona
    if musica:
        pygame.mixer.music.play(-1)
    ...
    ######### CONTRACAO TONICA
        ...
        if graf==1:
            ...
            if contagem>60:
                pygame.mixer.music.stop()
                sair=inicio()
            if contagem>=0 and contagem<60:
                if contagem==0.00:
                    pos_obs_y= altura*(1/2 - float(pos_col_y[0+cont*3]))
                obs1.show(tela, pos_obs_x, pos_obs_y)
                pos_obs_x-=velocidade_obs

                if pos_obs_x>=(largura-((largura%100)%10))/1.71 and pos_obs_x<=(largura-((largura%100)%10))/1.71+velocidade_obs: ##dist de 640 pixels entre os obstaculos
                    pos_obs_x2, pos_obs_y2 = larg, altura*(1/2 - float(pos_col_y[1+cont*3]))
                ...
                if pos_obs_x>=(largura-((largura%100)%10))/8.5 and pos_obs_x<=(largura-((largura%100)%10))/8.5+velocidade_obs: ##dist de 640 pixels entre os obstaculos
                    pos_obs_x3, pos_obs_y3 = larg, altura*(1/2 - float(pos_col_y[2+cont*3]))

                if pos_obs_x2<=round((largura-((largura%100)%10))/1.71): ##dist de 640 pixels entre os obstaculos
                    obs2.show(tela, pos_obs_x2, pos_obs_y2)
                    pos_obs_x2-=velocidade_obs
                if ((largura-((largura%100)%10))/8.5)+velocidade_obs<=pos_obs_x<round((largura-((largura%100)%10))/1.71): ##dist de 640 pixels entre os obstaculos
                    obs2.show(tela, pos_obs_x2, pos_obs_y2)
                    pos_obs_x2-=velocidade_obs
                if pos_obs_x3<=round((largura-((largura%100)%10))/1.71): ##dist de 640 pixels entre os obstaculos
                    obs3.show(tela, pos_obs_x3, pos_obs_y3)
                    pos_obs_x3-=velocidade_obs
                if -((round(largura-((largura%100)%10))//2.87))+velocidade_obs<=pos_obs_x<(largura-((largura%100)%10))/8.5: ##dist de 640 pixels entre os obstaculos
                    obs3.show(tela, pos_obs_x3, pos_obs_y3)
                    pos_obs_x3-=velocidade_obs

                if pos_obs_x<=-(round(largura-((largura%100)%10))//2.87) and pos_obs_x>=-(round(largura-((largura%100)%10))//2.87)-velocidade_obs: ##dist de 640 pixels entre os obstaculos
                    cont+=1
                    pos_obs_x=larg
                    pos_obs_y=altura*(1/2 - float(pos_col_y[0+cont*3]))
                print(pos_obs_x, pos_obs_y,pos_obs_x2, pos_obs_y2, pos_obs_x3, pos_obs_y3, contagem)
