...
def modo():
    ...
    pos_buble_y, col, cor, graf = 700, coluna(), coral(), 2
    ...
    while mod:
        ...
        ##botao_2 = TRABALHO ASCENDENTE
            ...
            elif posicao(11,15,"x") < mouse[0] < posicao(30,15,"x") and posicao(14,15,"y") < mouse[1] < posicao(14,17,"y") and click[0] == 1:
                pygame.mixer.Sound.stop(Inicio_sound)
               # graf=2
                fund.show(tela, posicao(18,18,"x"), posicao(18,18,"y"), 1)
                pygame.display.update()
        ...
        ##botao_3 = RELAXAMENTO
            ...
            elif posicao(11,9,"x") < mouse[0] < posicao(27,12,"x") and posicao(20,20,"y") < mouse[1] < posicao(22,22,"y") and click[0] == 1:
                pygame.mixer.Sound.stop(Inicio_sound)
               # graf=3
                fund.show(tela, posicao(18,18,"x"), posicao(18,18,"y"), 1)
                pygame.display.update()
        ...
        ##botao_4 = CONTRACAO TONICA
        
            elif posicao(11,9,"x") < mouse[0] < posicao(30,12,"x") and posicao(14,25,"y") < mouse[1] < posicao(22,27,"y") and click[0] == 1:
                pygame.mixer.Sound.stop(Inicio_sound)
               # graf=4
                fund.show(tela, posicao(18,18,"x"), posicao(18,18,"y"), 1)
                pygame.display.update()
...
#####################################
   ...
   # musica_bt.show(tela, -100, -100, 3)
    ...
    contagem=0
    if graf==1:
        pos_col_y = altura_col("Trabalho Isométrico.txt")
    elif graf==2:
        pos_col_y = altura_col("Trabalho Ascendente.txt")
    elif graf==3:
        pos_col_y = altura_col("Trabalho Concêntrico.txt")
    elif graf==4:
...
########### DETERMINAR ALTURA DAS COLUNAS
            if  pos_obs_x>(largura/3)-velocidade_obs and pos_obs_x<=(largura/3) and cont<=len(pos_col_y)-2: ##dist de 640 pixels entre os obstaculos
                cont+=1
                pos_obs_x2, pos_obs_y2 = largura, altura*(1/2 - pos_col_y[cont])
                #pos_obs_x2-=velocidade_obs

            if  pos_obs_x>-(largura/3)-velocidade_obs and pos_obs_x<=-(largura/3) and cont<=len(pos_col_y)-2:
                cont+=1
                pos_obs_x, pos_obs_y = largura, altura*(1/2 - pos_col_y[cont])

########### FIM DO GRÁFICO
            if cont==len(pos_col_y)-1 and pos_obs_x2<0:
                pygame.mixer.music.stop()
                sair=inicio()                    
