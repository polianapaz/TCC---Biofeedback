...
## Versão para integrar a ESP com comunicação BLE
...
#endereço BT da máquina onde o código python será executado
server_sock.bind(("F0:77:C3:27:B8:DD", bluetooth.PORT_ANY))
...
porta='COM'+str(port)
print(porta)
ser=serial.Serial("COM4", baudrate = 500000, timeout=0.01)
'''
...
def taragem():
    ...
    cont =  [letras()]*6 + [numeros()]*7 + [coluna(), coral(), sereia(),
             fundo(), bubble(), 700, 0, 720, 0, 0, 0, 0, altura,
             True, 2, 121]
    ...
    posimax, posimin, tara, velocidade_y, teste = [i for i in cont]
    ...
    while tara:
        ...
        while (contagem2 <= 10):
            ...
            if (ser.inWaiting() > 0):
                ...
                if (posiant > 120 and posi < 100):
                    posi=posi%100
                
                if (posi > posiant+40 and flag_transm == True):
                    posi = posiant+20

                while (flag_transm == False and (altura + 100 - 10*i > altura - posi*2.5)):
                        # precisa de alterações na contagem da posicao maxima, o objetivo
                        # é conseguir fazer o contador ir crescendo gradativamente até o valor de posi.
                        quant_loop = (altura + 100 - 10)/(altura - posi*2.5)
                        print("loop=", quant_loop)
                        fund.show(tela, posicao(18, 18,"x"), posicao(18, 18,"y"), 1)
                        cor.show(tela, posicao(35, 9,"x")+largura//36, posicao(35, 10,"y"), 1)
                        cor.show(tela, posicao(0, 9,"x"), posicao(0, 10,"y"), 1)
                        col.show(tela, posicao(35, 23,"x")+largura//36, posicao(35, 24,"y"))
                        col.show(tela, posicao(0, 23,"x"), posicao(0, 24,"y"))
                        test3.show(tela, 3, 5, "MAX", 1)
                        posimaxcen.show(tela, posicao(5, 8,"x"), posicao(3, 8,"y"), (int(posi*(i/quant_loop)) % 100)//10)
                        posimaxdez.show(tela, posicao(6, 8,"x"), posicao(3, 8,"y"), ((int(posi*(i/quant_loop)) % 100)//10))
                        posimaxuni.show(tela, posicao(7, 8,"x"), posicao(3, 8,"y"), ((int(posi*(i/quant_loop)) % 100)%10))

                        print (int(posi*(i/quant_loop)))
                        if pos_buble_y == altura:
                            nova = randint(-300, 300)
                            nova2 = randint(-300, 300)
                        ...
                        ariel.show(tela, posicao(18, 18,"x"), altura + 100 - 10*i, i%2)
                        i+=1
                        clock.tick(15)
                        pygame.display.update()
            ...
            if (pos_buble_y % 15)==0:
                ...
                escolha = 0
            if(flag_transm == True):
                ariel.show(tela, posicao(18, 18,"x"), altura - posi*2.5, escolha)
            if posi > posimax:
                posimax = posi
        ...
        while (contagem3 <= 12):
            ...
            if (ser.inWaiting()>0):
                ...
                if (posiant > 120 and posi2 < 100):
                    posi2=posi2%100

                if (posi2 > posiant+40 and flag_transm == True):
                    posi2 = posiant+10
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ser.write(str.encode("p"))
                    tara = False
            ariel.show(tela, posicao(18, 18,"x"), altura - posi2*2, escolha)
            if posi2 < posimin:
                posimin = posi2
